...
// Retry hop: a failed message parks here for RETRY_DELAY_MS, then TTL expiry dead-letters it
// back into MAIL_EXCHANGE (same routing key, so it lands on courier-one.queue again) for
// another attempt.
...
// Give-up destination: after MAX_RETRIES, a message goes here instead of back to the retry
// queue - a fanout exchange so it's caught regardless of the original routing key.
...
export async function consumeMailEvents(handler) {
  ...
  // Retry queue - nothing ever consumes this directly; it's a parking lot that expires
  // messages back into the main exchange after RETRY_DELAY_MS.
  ...
  channel.consume(queue, async (msg) => {
    ...
    try {
      ...
      if (attempt > MAX_RETRIES) {
        ...
        // nack would send it back through the queue's own x-dead-letter-exchange (the retry
        // queue) again - publish straight to the final DLQ instead, then ack to remove it
        // from the main queue without re-triggering that path.
        ...
        // Not requeued on the same queue (would spin immediately) - goes to the retry queue
        // instead, via x-dead-letter-exchange, and comes back after RETRY_DELAY_MS.
...
// RabbitMQ merges repeat dead-letterings of the same message from the same queue+reason into
// one x-death entry with an incrementing count, rather than appending a new entry each time -
// so this reflects the true number of failed attempts regardless of how many retry hops happened.
