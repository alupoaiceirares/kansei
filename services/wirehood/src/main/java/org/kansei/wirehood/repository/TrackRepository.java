...
public interface TrackRepository extends ReactiveCrudRepository<Track, UUID> {
    ...
    // Song of the Day pick - readiness lives on track_formats now (per format), not tracks (tracks has no status
    // column anymore); visible=true excludes anything an admin's hidden from the pool. DISTINCT since a track
    // with two READY formats (mp3 + mp4) would otherwise join into two rows.
    @Query("""
